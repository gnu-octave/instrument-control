...
## -*- texinfo -*-
## @deftypefn {Function File} {} configureTerminator (@var{dev}, @var{term})
## @deftypefnx {Function File} {} configureTerminator (@var{dev}, @var{readterm}, @var{writeterm})
## Set terminator for ASCII string manipulation
##
## @subsubheading Inputs
## @var{dev} - visadev object@*
...
function configureTerminator (dev, readterm, writeterm)

  if nargin < 2
    error ("Expected terminal");
  elseif nargin == 2
    __visadev_dispatch__ (dev, 'terminator', readterm);
  elseif nargin == 3
    __visadev_dispatch__ (dev, 'terminator', readterm, writeterm);
  else
