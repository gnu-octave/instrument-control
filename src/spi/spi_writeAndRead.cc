  ...
  if (args (1).is_uint8_type ()) // uint8_t
    {
      ...
      if (buf == NULL)
        {
          error ("spi_writeAndRead: cannot allocate requested memory: %s", strerror (errno));
          return octave_value (-1);  
        }
      ...
      if (rdbuffer == NULL)
        {
          error ("spi_writeAndRead: cannot allocate requested read memory: %s\n", strerror (errno));
          return octave_value (-1);  
        }

      retval = spi->writeRead (buf, data.numel(), rdbuffer);
	  ...
	  return return_value;
        ...
        error ("spi_writeAndRead: error reading data");
    }
  else
    {
      error ("spi_writeAndRead: expected uint8 data");
      return octave_value ();
    }
